...
def get_material_list(db: Session):
    material_list = (
        db.query(Material)
        .order_by(Material.type.asc(), Material.name_ko.asc(), Material.name.asc())
        .all()
    )
...
def update_material(
    db: Session, db_material: Material, material_update: material_schema.MaterialUpdate
):
...
def get_material_by_spirit(db: Session, spirits: list):
    ...
    materials = (
        db.query(Material.type, Material.name, Material.name_ko)
        .distinct()
    ...
    materials = [
        material_schema.MaterialOmittedUnitAmount(
            type=material.type.value,
            name=material.name.replace("_", " "),
            name_ko=material.name_ko.replace("_", " "),
        )
