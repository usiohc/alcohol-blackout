...
@router.get(
    "/{cocktail_id:int}/spirits", response_model=cocktail_schema.CocktailSpirits
)
def cocktail_spirit_list(cocktail_id: int, db: Session = Depends(get_db)):
    ...
    total, _cocktail_spirit_list = cocktail_crud.get_cocktail_spirit_list(
        db, cocktail_id=cocktail_id
    )
    if not _cocktail_spirit_list:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="등록된 Spirit이 없습니다."
        )
    return {"total": total, "spirits": _cocktail_spirit_list}


@router.get(
    "/{cocktail_id:int}/materials", response_model=cocktail_schema.CocktailMaterials
)
def cocktail_material_list(cocktail_id: int, db: Session = Depends(get_db)):
    ...
    total, _cocktail_material_list = cocktail_crud.get_cocktail_material_list(
        db, cocktail_id=cocktail_id
    )
...
@router.post(
    ...
    response_model=cocktail_schema.Cocktail,
)
def cocktail_create(
    _cocktail_create: cocktail_schema.CocktailCreate,
    ...
    """
    created_cocktail = cocktail_crud.create_cocktail(db=db, cocktail=_cocktail_create)
...
def cocktail_update(
    ...
    updated_cocktail = cocktail_crud.update_cocktail(
        db=db, db_cocktail=cocktail, cocktail_update=_cocktail_update
    )
...
def cocktail_by_spirit_material(
    spirits: str | None = Query("", convert_underscores=False),
    ...
    total, cocktails = cocktail_crud.get_cocktail_by_spirit_material(
        spirits=spirits, materials=materials, db=db
    )
    if not cocktails:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Cocktail을 찾을 수 없습니다."
        )
    return {"total": total, "items": cocktails}


@router.get("/{cocktail_name:str}", response_model=cocktail_schema.CocktailDetailByName)
def cocktail_detail_by_name(cocktail_name: str, db: Session = Depends(get_db)):
    """
    Frontend 칵테일 상세 페이지
    """
    cocktail_name = cocktail_name.replace(" ", "_")
    cocktail = cocktail_crud.get_cocktail_detail_by_name(
        db=db, cocktail_name=cocktail_name
    )
    if not cocktail:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
def cocktail_request(_cocktail_request: cocktail_schema.CocktailRequest):
    ...
    if not upload_blob_from_memory(
        bucket_name="acbo-request_recipes",
        data=_cocktail_request,
        destination_blob_name=filename,
    ):
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="칵테일 레시피를 요청하는데 실패했습니다."
            "관리자에게 문의해주세요.",
        )
    return None
