...
def test_user_register(client):
    ...
    response = client.post("/api/oauth2/token",
...
def test_user_login_me_update(client, verified_user):
    ...
    response = client.patch("/api/users/username",
                            json={"username": "new_username"},
                            headers={"Authorization": f"Bearer {res['access_token']}"})
    assert response.status_code == 200

    response = client.patch("/api/users/password",
                            json={"password": "new_password", "passwordCheck": "new_password"},
                            headers={"Authorization": f"Bearer {res['access_token']}"})
    assert response.status_code == 200

    response = client.get("/api/users/me", headers={"Authorization": f"Bearer {res['access_token']}"})
    res = response.json()
    assert response.status_code == 200
    assert res['email'] == verified_user.email
    assert res['username'] == verified_user.username
...
def test_user_delete(client, verified_user):
    ...
    response = client.delete("/api/users", headers={"Authorization": f"Bearer {res['access_token']}"})
    assert response.status_code == 200
